Ext.define('Zenws.zenws.web.com.zen.controller.app.ClubMainController', {
     extend: 'Zenws.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.ClubMainController',
     init: function() {
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var tree = this.view.down('#ClubTree');
               ...
               var compRef = this.view.down('#ClubTree');
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.clubId;
          me = this;
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#Club');
          var grid = this.view.down('#ClubGrid');
          var tree = this.view.down('#ClubTreeContainer');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, currentObject) {
                    ...
                    var compRef = scope.sender.view.down('#ClubTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var Club = this.view.down('#ClubForm');
               Club.reset();
               var gridPanel = this.view.down('#ClubGrid');
          ...
          var formPanel = this.getView().up().down('#ClubForm');
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) { /** Finding record by id */
          ...
          var treePanel = this.view.up().up().down('#ClubTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#Club').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#ClubGrid').store;
          gridStore.removeAll();
          gridStore.add(data);
     ...
     loadData: function(responseData) {
          this.view.down('#ClubForm').getViewModel().setData(responseData.response.data);
     },
